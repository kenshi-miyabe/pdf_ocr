...
- `--timeout`: ページごとの API リクエストのタイムアウト秒数です。既定値は 1200 秒です
...
- stream で受け取った thinking があり、timeout または空応答になった場合は、PDF と同じディレクトリの `thinking-<PDFファイル名のbasename>.txt` に追記します
